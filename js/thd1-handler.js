...
document.getElementById('btn-lihat-undangan').onclick = function() {
    enableScroll();
    setTimeout(function(){$('#greeting-icon').tooltip('show')}, 7000);
    setTimeout(function(){$('#greeting-icon').tooltip('hide')}, 20000);
};
...
var fileKey = '1v-e4tLcGyMGk8Ek9z5PiJNSfKAxbXsHY';
    ...
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error("Error fetching the dynamic key:", textStatus, errorThrown);
...
function loadAudio() {
    return new Promise(async (resolve, reject) => {
        ...
        if (!('caches' in window)) {
            console.warn('Cache API tidak didukung di browser ini. Memuat langsung dari sumber.');
            try {
        ...
        try {
            ...
            if (!networkResponse.ok) {
                throw new Error(`Gagal mengambil audio: ${networkResponse.statusText} (status: ${networkResponse.status})`);
            }
...
$(document).ready(function() {
    ...
    function playAudio() {
        audioReadyPromise.then(() => {
            ...
            console.warn("Tidak dapat memutar musik karena proses pemuatan gagal:", error);
            audioIcon.setAttribute('class', 'icon-music');
        });
    }
...
$(document).ready(function() {
    ...
    $(document).delegate("[data-target='#modal-preview-reply']", "click", function() {
        ...
        $.ajax({
            ...
            success: function(jumlahUndangan) {
                if (jumlahUndangan > defaultShowNumber) {
                    $('#button-show-all').show();
                    $('#count-undangan').text('('+ (jumlahUndangan - defaultShowNumber) +')');
                }
            }
        });
    }
    ...
    function formatDate(date) {
        ...
        return [day, months[month], year].join(' ') + ' - ' + [d.getHours(), d.getMinutes()].join(':');
    }
});
...
$('.footer-icon').on('click', function() {
    ...
    $.ajax({
        url: 'features/service.php?action=resell&resellCode='+ encodeURIComponent(resellCode) +'&media='+media,
        type: 'post',
        async: true,
        success: function(data) {
...
// ----- DATE COUNTER ----- //
// var end = new Date('09/29/2025 02:00 PM');
var end = new Date();
